...
class Sample(object):
  def __init__(self, width = 800, height = 600):
    ...
    sphere.color = (numpy.array((0., 0., 1.), dtype=numpy.float32))
    sphere.diffuse = (1.)
    sphere.reflection = (.5)
    self.scene.addPrimitive(sphere)
    sphere = IRT.Sphere(numpy.array((2., 1., 15.), dtype=numpy.float32), 1.)
    sphere.color = (numpy.array((1., 0., 0.), dtype=numpy.float32))
    sphere.diffuse = (1.)
    sphere.reflection = (.5)
    self.scene.addPrimitive(sphere)
    sphere = IRT.Sphere(numpy.array((-2., -1., 15.), dtype=numpy.float32), 1.)
    sphere.color = (numpy.array((.0, 1., .0), dtype=numpy.float32))
    sphere.reflection = (.5)
    sphere.diffuse = (1.)
    self.scene.addPrimitive(sphere)

    box = IRT.Box(numpy.array((-8., 4., 26.), dtype=numpy.float32), numpy.array((8., 10., 1000.), dtype=numpy.float32))
    box.color = (numpy.array((.95, .95, .95), dtype=numpy.float32))
    box.reflection = (.25)
    ...
    triangle.color = (numpy.array((1., 1., 1.), dtype=numpy.float32))
