...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_getBoundingBox )
{
  ...
  BOOST_CHECK(all(bb.corner1 == scene_bb.corner1));
  BOOST_CHECK(all(bb.corner2 == scene_bb.corner2));
...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_removePrimitive )
{
  ...
  Primitive* primitive2 = new Sphere(Vector3df(0.), 3.f);
...
{
  Light* light = new Light(Vector3df(0.), Vector3df(1.));
  SimpleScene* scene = new SimpleScene;
...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_removeLight )
{
  ...
  BOOST_REQUIRE_EQUAL(light, result);
  Light* light2 = new Light(Vector3df(0.), Vector3df(1.));
...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_getFirstCollision )
{
  ...
  BOOST_CHECK_EQUAL(primitive, scene->getFirstCollision(Ray(Vector3df(-5.), Vector3df(1.)), dist, tnear, tfar));
  BOOST_CHECK_GT(dist, 0);
  BOOST_CHECK_EQUAL(scene->getFirstCollision(Ray(Vector3df(-5.), Vector3df(-1.)), dist, tnear, tfar), static_cast<Primitive*>(NULL));
...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_testCollision )
{
  ...
  BOOST_CHECK(scene->testCollision(Ray(Vector3df(-5.), Vector3df(1.)), std::numeric_limits<float>::max()));
  BOOST_CHECK(!scene->testCollision(Ray(Vector3df(-5.), Vector3df(-1.)), std::numeric_limits<float>::max()));
...
BOOST_AUTO_TEST_CASE( test_IRT_SimpleScene_computeColor )
{
  ...
  Light* light = new Light(Vector3df(5.), Vector3df(1.));
  SimpleScene* scene = new SimpleScene;
  ...
  BOOST_CHECK(all(scene->computeColor(Vector3df(4.), material, primitive) > 0.f));
  BOOST_CHECK(all(scene->computeColor(Vector3df(-4.), material, primitive) == 0.f));
